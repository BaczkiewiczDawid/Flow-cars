...
export const ownedCars = pgTable('owned_cars', {
  ...
  insuranceExpiresAt: timestamp('insurance_expires_at', { mode: 'date' }),
  inspectionExpiresAt: timestamp('inspection_expires_at', { mode: 'date' }),
  reregisteredAt: timestamp('reregistered_at', { mode: 'date' }),
  createdAt: timestamp('created_at', { mode: 'date' }).notNull(),
  updatedAt: timestamp('updated_at', { mode: 'date' }).notNull(),
  userId: uuid('user_id').references(() => users.id),
});
